...
class ClassGenerator
{
    ...
    private function addConditionals($class)
    {
        ...
        foreach ($this->conditionals as $name => $conditional) {
            ...
            // split up param name if needed
            $paramName = $this->conditionalParamMap[$name];
            $paramAndMode = '"and"';
            ...
            $paramMethodCall = '$this->hasParam(["' . $paramName . '"], ' . $paramAndMode . ')';

            $body = '
				if (($isNegated === false && !' . $paramMethodCall . ') ||
				    ($isNegated === true && ' . $paramMethodCall . ')
				) {
					return self::EMPTY_STRING;
				}
			' . PHP_EOL . 'return ' . $conditional . ';';

            $method = $class->addMethod('getConditional' . $name)
                ->setBody($body);

            $method->addParameter('isNegated', false);
        }
